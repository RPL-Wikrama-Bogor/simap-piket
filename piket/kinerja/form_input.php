...
//end logika menentukan semester
  ...
  if (isset($_POST['simpan'])) {
    $nisku = @$_POST['nis'];
    $tglku = @$_POST['tgl_kejadian'];
    $kodeku = @$_POST['kode_kinerja'];
    $saksiku = @$_POST['saksi'];

    $qw  = mysql_query("SELECT * FROM tbl_kinerja_siswa WHERE nis = '$nisku' AND tgl_kejadian = '$tglku' AND kode_kinerja = '$kodeku' AND saksi = '$saksiku' ");
    $cek = mysql_num_rows($qw);
    if ($cek > 0) {
          echo "<script>window.alert('Tidak bisa menginput di hari dan saksi yang sama, kecuali berbeda kinerja')
       </script>";
    }else{
      $aksi->simpan($table,$field,$redirect);
      $aksi->alert("Data Berhasil Disimpan",$redirect);
      // echo "<script>window.alert('Jaja ')</script>";
